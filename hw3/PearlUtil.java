...
public class PearlUtil {
	...
	public void collectPearls(State[] states, MoveRecord[] records, int start, int end) {
		for (int i = start; i <= end; i += 1) {
			if (states[i] == PEARL) {
				...
				// for every pearl found in states array, set it to disappeared in the
				// moveRecord
	...
	public int movePlayer(State[] states, MoveRecord[] records, Direction dir) {
		...
		// sets first element of records array to record where the player has moved to
		...
		while (!done) {
			...
			if (nextMovable < 0) {
				...
				if (prev >= 0 && State.canMerge(states[nextMovable], states[prev])) {
					...
					// sets records array to show the blocks moved to their new indexes or
					// disappeared
